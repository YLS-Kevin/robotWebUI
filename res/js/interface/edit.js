...
$(function() {
	...
	$(".t1>a").html(data.explains);
	$("input.inter_name").val(data.explains); //接口名称
	$("input.url").val(data.url); //生产url
	$("input.test_url").val(data.urltest); //测试url
	...
	layui.use(['element', 'form', 'layedit', 'layer'], function() {
		...
		form.render();
		//渲染请求方式
		$("#ajaxtype").next().find('.layui-this').removeClass('layui-this');
		$("div#icall").find("dd").each(function() {
			if(data.icall == $(this).attr("lay-value")) {
				$("#ajaxtype").next().find("div input").val($(this).html());
				$(this).addClass('layui-this');
				$(this).siblings().removeClass('layui-this');
			}
		})
		//渲染是否终端
		$("#isFrontCall").next().find('.layui-this').removeClass('layui-this');
		$("div#isip").find("dd").each(function() {
			if(data.isFrontCall == $(this).attr("lay-value")) {
				$("#isFrontCall").next().find("div input").val($(this).html());
				$(this).addClass('layui-this');
				$(this).siblings().removeClass('layui-this');
			}
		})
		...
		form.on('submit(submitAdd)', function(e) {
			...
			getAjax($ctx + "modifyInterface", {
				...
				if(data.ret == 0) {
					layer.alert("修改数据接口成功", function(index) {
	...
	//参数的渲染
	var paramName = data.paramName.replace(/[\s\|]/g, ",").split(",");
	if(data.paramName.length == 0) {

	} else {
		var tag1HTML = "";
		for(var i = 0; i < paramName.length; i++) {
			tag1HTML = '<span class="tag-label">' + paramName[i] + '<i class="layui-icon icon_close layui-icon-close"></i></span>'
			$("input#param").before(tag1HTML);
			arrParam.push(paramName[i]);
		}
	...
	//参数值的渲染
	var paramValue = data.paramValue.replace(/[\s\|]/g, ",").split(",");
	if(data.paramValue.length == 0) {

	} else {
		var tag2HTML = "";
		for(var i = 0; i < paramValue.length; i++) {
			tag2HTML = '<span class="tag-label">' + paramValue[i] + '<i class="layui-icon icon_close layui-icon-close"></i></span>'
			$("input#paramValue").before(tag2HTML);
			arrParamValue.push(paramValue[i]);
		}		
